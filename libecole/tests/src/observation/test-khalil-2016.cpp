...
TEST_CASE("Khalil2016 return correct observation", "[obs]") {
	...
	SECTION("Observation has correct values") {
		...
		SECTION("Min/max for ratios of constraint coeffs. to RHS") {
			...
			REQUIRE(
				xt::all(col(Dynamic::coef_pos_rhs_ratio_min) <= col(Dynamic::coef_pos_rhs_ratio_max)));
			...
			REQUIRE(
				xt::all(col(Dynamic::coef_neg_rhs_ratio_min) <= col(Dynamic::coef_neg_rhs_ratio_max)));
		}

		SECTION("Stats. for active constraint coefficients") {
			REQUIRE(xt::all(col(Dynamic::active_coef_weight1_count) >= 0));
			REQUIRE(
				xt::all(col(Dynamic::active_coef_weight1_mean) >= col(Dynamic::active_coef_weight1_min)));
			REQUIRE(
				xt::all(col(Dynamic::active_coef_weight1_mean) <= col(Dynamic::active_coef_weight1_max)));
			REQUIRE(xt::all(col(Dynamic::active_coef_weight2_count) >= 0));
			REQUIRE(
				xt::all(col(Dynamic::active_coef_weight2_mean) >= col(Dynamic::active_coef_weight2_min)));
			REQUIRE(
				xt::all(col(Dynamic::active_coef_weight2_mean) <= col(Dynamic::active_coef_weight2_max)));
			REQUIRE(xt::all(col(Dynamic::active_coef_weight3_count) >= 0));
			REQUIRE(
				xt::all(col(Dynamic::active_coef_weight3_mean) >= col(Dynamic::active_coef_weight3_min)));
			REQUIRE(
				xt::all(col(Dynamic::active_coef_weight3_mean) <= col(Dynamic::active_coef_weight3_max)));
			REQUIRE(xt::all(col(Dynamic::active_coef_weight4_count) >= 0));
			REQUIRE(
				xt::all(col(Dynamic::active_coef_weight4_mean) >= col(Dynamic::active_coef_weight4_min)));
			REQUIRE(
				xt::all(col(Dynamic::active_coef_weight4_mean) <= col(Dynamic::active_coef_weight4_max)));
		}
	}
}
